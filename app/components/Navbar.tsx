...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      if (!(e.target as Element).closest("[data-social-dropdown]")) setSocialOpen(false);
    };
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      if (!(e.target as Element).closest("[data-account-dropdown]")) setAccountOpen(false);
    };
    document.addEventListener("mousedown", close);
    return () => document.removeEventListener("mousedown", close);
  }, [accountOpen]);
  ...
  return (
    <header
      ...
      style={{ background: "rgba(13,21,32,0.92)" }}
    >
      <div className="ocws-container py-3">
        <div className="flex items-center justify-between gap-4">
                ...
                {accountOpen && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-white/[0.06] transition mx-1 rounded-lg w-full text-left"
                      style={{ color: "#F87171" }}
                    >
                      <span>↩</span>
                      <span>Sign Out</span>
          ...
          {/* Right: Get the App CTA + hamburger */}
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              <span className="relative block h-4 w-5">
                <span className={`absolute left-0 top-0 h-[2px] w-5 bg-white/85 transition-transform duration-200 ${open ? "translate-y-[7px] rotate-45" : ""}`} />
                <span className={`absolute left-0 top-[7px] h-[2px] w-5 bg-white/85 transition-opacity duration-200 ${open ? "opacity-0" : "opacity-100"}`} />
                <span className={`absolute left-0 bottom-0 h-[2px] w-5 bg-white/85 transition-transform duration-200 ${open ? "-translate-y-[7px] -rotate-45" : ""}`} />
              </span>
            </button>
          </div>
        </div>
