...
const ACCESS_DESCRIPTIONS: Record<string, string> = {
  fledgling: "You'll see exactly what's wrong with your wireless — and how serious it is. Fix details unlock with a full WiFi Health Report.",
  nest: 'Full single-location WiFi Health Report. Corvus finds every problem and tells you exactly how to fix it.',
  flock: 'Full WiFi Health Report plus Whole-Home Survey for multi-location properties. PDF report included.',
...
export default function DemoLandingPage() {
  ...
  if (status === 'invalid' || status === 'expired') return (
    <div style={s.page}><div style={s.card}>
      ...
      <p style={{ color: '#888', fontSize: '0.85rem' }}>Need access? Visit <a href="https://oldcrowswireless.com" style={{ color: '#22D6DC' }}>oldcrowswireless.com</a></p>
    </div></div>
  )
  ...
  return (
    <div style={s.page}><div style={s.card}>
      ...
      <p style={s.footer}>
        Powered by <span style={{ color: '#22D6DC' }}>Old Crows Wireless Solutions</span><br />
...
const s: Record<string, React.CSSProperties> = {
  ...
  card: { background: '#1A2332', border: '1px solid rgba(34,214,220,0.2)', borderRadius: '16px', padding: '2.5rem 2rem', maxWidth: '440px', width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' },
  badge: { background: 'rgba(34,214,220,0.12)', border: '1px solid rgba(34,214,220,0.3)', color: '#22D6DC', borderRadius: '20px', padding: '4px 16px', fontSize: '0.85rem', fontFamily: 'Share Tech Mono, monospace' },
  ...
  timer: { color: '#D8AC32', fontSize: '0.85rem', fontFamily: 'Share Tech Mono, monospace' },
  uses: { color: '#888', fontSize: '0.85rem', fontFamily: 'Share Tech Mono, monospace' },
  launchBtn: { background: 'linear-gradient(135deg, #0D6E7A, #22D6DC)', color: '#fff', border: 'none', borderRadius: '8px', padding: '14px 32px', fontSize: '1rem', fontWeight: 700, cursor: 'pointer', width: '100%' },
