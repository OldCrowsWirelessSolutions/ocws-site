...
const DEMO_LINES = [
  "I've already rendered my WiFi Health Report. You're just here for the sentencing.",
  ...
  "Two WiFi networks fighting over the same channel. Only one of them is yours.",
  "I don't render opinions. I render WiFi Health Reports. There's a difference. You'll understand shortly.",
]
...
const FEATURES = [
  ...
  { icon: '⚡', title: 'WiFi Health Report in Under 5 Min', desc: 'No technician. No appointment. No $75 visit fee. Just answers.' },
  { icon: '🔧', title: 'Router-Specific Fixes', desc: 'Step-by-step instructions for your exact hardware. Not generic advice.' },
  { icon: '📄', title: 'PDF WiFi Health Report', desc: 'Branded, professional, shareable. Suitable for landlords and vendors.' },
  { icon: '🏢', title: 'Whole-Home Survey', desc: 'Up to 15 locations synthesized into one comprehensive analysis.' },
]
  ...
  return (
    <div style={s.page}>
      <div style={s.hero}>
        ...
        <h2 style={s.heroSub}>The only AI that renders WiFi Health Reports on your wireless network.</h2>
      ...
      <div style={s.section}>
        ...
        <div style={s.scanCard}>
          {!scanning && !showVerdict && (
            <>
              <p style={s.scanDesc}>This is a demonstration with sample data from a real retail location. Hit the button and watch Corvus render a WiFi Health Report in real time.</p>
          ...
          {scanning && (
            <div style={s.scanProgress}>
              ...
              <p style={s.scanProcessing}>
                {scanProgress < 40 ? 'Parsing signal environment...' : scanProgress < 60 ? 'Identifying channel conflicts...' : scanProgress < 80 ? 'Cross-referencing vendor signatures...' : 'Rendering WiFi Health Report...'}
              </p>
            </div>
          )}
          {showVerdict && (
            <div>
              <div style={s.verdictHeader}>
                ...
                <div style={s.verdictTitle}>WIFI HEALTH REPORT</div>
              ...
              <div style={s.findingsList}>
                {[
                  ...
                  { n: 2, title: 'Business WiFi Network on 2.4 GHz Only — No 5 GHz Offload', sev: 'high' },
                  { n: 3, title: 'Guest Network Broadcasting Same Name as Business WiFi', sev: 'high' },
                ].map(f => (
                  <div key={f.n} style={s.finding}>
                    <div style={{ ...s.findingSev, color: f.sev === 'critical' ? '#e05555' : '#D8AC32' }}>{f.sev.toUpperCase()}</div>
                    <div style={s.findingTitle}>{f.n}. {f.title}</div>
                  </div>
                ))}
              </div>
              <div style={s.verdictGate}>
                <p style={s.gateText}>Full fix instructions, router-specific steps, and PDF report unlock with a WiFi Health Report.</p>
      ...
      <div style={s.cta}>
        ...
        <a href="/crows-eye" style={s.ctaBtn}>Get Your WiFi Health Report — $50</a>
...
const s: Record<string, React.CSSProperties> = {
  ...
  quoteBox: { background: '#1A2332', border: '1px solid rgba(34,214,220,0.25)', borderRadius: '12px', padding: '1.5rem 2rem', maxWidth: '600px', width: '100%' },
  quoteMark: { color: '#22D6DC', fontSize: '3rem', lineHeight: 1, fontFamily: 'Georgia, serif', marginBottom: '-0.5rem', opacity: 0.4 },
  ...
  sectionTitle: { color: '#22D6DC', fontFamily: 'Share Tech Mono, monospace', fontSize: '0.85rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '1.5rem' },
  featureGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' },
  featureCard: { background: '#1A2332', border: '1px solid rgba(34,214,220,0.1)', borderRadius: '10px', padding: '1.25rem' },
  ...
  scanCard: { background: '#1A2332', border: '1px solid rgba(34,214,220,0.2)', borderRadius: '12px', padding: '2rem', maxWidth: '580px' },
  scanDesc: { color: '#aaa', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '1.25rem' },
  scanBtn: { background: 'linear-gradient(135deg, #0D6E7A, #22D6DC)', color: '#fff', border: 'none', borderRadius: '8px', padding: '13px 28px', fontSize: '1rem', fontWeight: 700, cursor: 'pointer' },
  scanProgress: { textAlign: 'center' },
  scanLabel: { color: '#22D6DC', fontFamily: 'Share Tech Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.15em', marginBottom: '1rem' },
  progressBar: { background: '#0D1520', borderRadius: '4px', height: '6px', overflow: 'hidden', marginBottom: '0.5rem' },
  progressFill: { background: 'linear-gradient(90deg, #0D6E7A, #22D6DC)', height: '100%', transition: 'width 0.8s ease' },
  scanPct: { color: '#D8AC32', fontFamily: 'Share Tech Mono, monospace', fontSize: '1.2rem', fontWeight: 700 },
  ...
  verdictTitle: { color: '#22D6DC', fontFamily: 'Share Tech Mono, monospace', fontSize: '0.75rem', letterSpacing: '0.15em' },
  verdictClient: { color: '#888', fontSize: '0.82rem', marginTop: '0.25rem' },
  verdictSummary: { background: '#0D1520', borderLeft: '3px solid #22D6DC', padding: '0.75rem 1rem', color: '#aaa', fontSize: '0.88rem', fontStyle: 'italic', lineHeight: 1.6, marginBottom: '1rem', borderRadius: '0 6px 6px 0' },
  ...
  verdictGate: { background: 'rgba(34,214,220,0.06)', border: '1px dashed rgba(34,214,220,0.2)', borderRadius: '10px', padding: '1.25rem', textAlign: 'center', marginBottom: '1rem' },
  gateText: { color: '#888', fontSize: '0.85rem', marginBottom: '0.75rem' },
  gateBtn: { display: 'block', background: 'linear-gradient(135deg, #0D6E7A, #22D6DC)', color: '#fff', borderRadius: '8px', padding: '12px', textDecoration: 'none', fontWeight: 700, marginBottom: '0.5rem', fontSize: '0.95rem' },
  gateLink: { color: '#22D6DC', fontSize: '0.82rem', textDecoration: 'none' },
  ...
  ctaBtn: { background: 'linear-gradient(135deg, #0D6E7A, #22D6DC)', color: '#fff', borderRadius: '8px', padding: '14px 32px', textDecoration: 'none', fontWeight: 700, fontSize: '1rem' },
  ctaLink: { color: '#22D6DC', fontSize: '0.85rem', textDecoration: 'none' },
}
