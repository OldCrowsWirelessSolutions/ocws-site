...
const SEVERITY_COLORS: Record<string, string> = {
  critical: '#e05555', high: '#D8AC32', moderate: '#22D6DC', info: '#888',
}
const SEVERITY_BG: Record<string, string> = {
  critical: 'rgba(224,85,85,0.1)', high: 'rgba(216,172,50,0.1)', moderate: 'rgba(34,214,220,0.08)', info: 'rgba(136,136,136,0.08)',
}
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        <div style={s.headerMeta}>
          ...
          <span style={s.metaTag}>{cs.product === 'reckoning' ? `Whole-Home Survey — ${cs.locations} Locations` : "WiFi Health Report"}</span>
        </div>
      ...
      <div style={s.statsBar}>
        ...
        {highCount > 0 && <div style={s.stat}><div style={{ ...s.statNum, color: '#D8AC32' }}>{highCount}</div><div style={s.statLabel}>High Priority</div></div>}
        <div style={s.stat}><div style={{ ...s.statNum, color: '#22D6DC' }}>{'< 5 min'}</div><div style={s.statLabel}>Time to Report</div></div>
      </div>
      ...
      {cs.comfortLevels && cs.comfortLevels.length > 0 && (
        <div style={s.section}>
          ...
          <p style={s.comfortIntro}>This Whole-Home Survey was run five times — one at each comfort level. Same building. Same problems. Watch how Corvus adapts his voice.</p>
      ...
      {cs.pdfFile && (
        <div style={s.section}>
          <a href={`/${cs.pdfFile}`} target="_blank" rel="noopener noreferrer" style={s.pdfBtn}>📄 Download Full Report PDF</a>
        </div>
      )}

      <div style={s.cta}>
        <h3 style={s.ctaTitle}>Ready for your own WiFi Health Report?</h3>
...
const s: Record<string, React.CSSProperties> = {
  ...
  breadLink: { color: '#22D6DC', fontSize: '0.82rem', textDecoration: 'none' },
  ...
  metaTag: { background: 'rgba(34,214,220,0.08)', border: '1px solid rgba(34,214,220,0.2)', color: '#22D6DC', borderRadius: '12px', padding: '3px 10px', fontSize: '0.75rem', fontFamily: 'Share Tech Mono, monospace' },
  ...
  corvusBlock: { background: '#1A2332', borderLeft: '3px solid #22D6DC', padding: '1.25rem', borderRadius: '0 8px 8px 0' },
  corvusBlockLabel: { color: '#22D6DC', fontSize: '0.68rem', fontFamily: 'Share Tech Mono, monospace', letterSpacing: '0.12em', marginBottom: '0.5rem' },
  ...
  fixLabel: { color: '#22D6DC', fontSize: '0.72rem', fontFamily: 'Share Tech Mono, monospace', letterSpacing: '0.08em' },
  ...
  levelTab: { background: 'transparent', border: '1px solid rgba(34,214,220,0.2)', color: '#888', borderRadius: '6px', padding: '6px 14px', fontSize: '0.82rem', cursor: 'pointer' },
  levelTabActive: { background: 'rgba(34,214,220,0.12)', color: '#22D6DC', borderColor: '#22D6DC' },
  ...
  testimonialBlock: { background: '#1A2332', borderRadius: '10px', padding: '1.5rem', border: '1px solid rgba(216,172,50,0.2)' },
  testimonialText: { color: '#ccc', fontSize: '1rem', lineHeight: 1.7, fontStyle: 'italic', margin: '0 0 0.75rem' },
  testimonialAuthor: { color: '#D8AC32', fontSize: '0.82rem', fontFamily: 'Share Tech Mono, monospace' },
  pdfBtn: { display: 'inline-block', background: 'rgba(34,214,220,0.08)', border: '1px solid rgba(34,214,220,0.25)', color: '#22D6DC', borderRadius: '8px', padding: '12px 20px', textDecoration: 'none', fontSize: '0.9rem' },
  ...
  ctaBtn: { display: 'inline-block', background: 'linear-gradient(135deg, #0D6E7A, #22D6DC)', color: '#fff', borderRadius: '8px', padding: '13px 28px', textDecoration: 'none', fontWeight: 700, fontSize: '0.95rem' },
}
